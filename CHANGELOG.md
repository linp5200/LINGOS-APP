...
## [0.7.1] - 2026-09-25（hotfix：安全/链路完善批次）

### 新增
- 首启引导（onboarding——欢迎/连接/就绪清单 三步）
- 服务器模式控制屏（远程 status/on/stop——危机时停止自动禁用）
- 天气城市管理（weather_set_city）· 危机投递状态显示（crisis_delivery_status）
- 审批三态（允许一次 / **始终允许**（服务端记忆）/ 拒绝）
- 语音连续对话（voice_continuous_chat 接线——循环录音/STT/发送/朗读）
- 主控台双态升级：连接后变 server 真数据（CPU/内存/磁盘/版本——5s 刷新）

### 修复 / 接线
- 通知开关接线（notif_alert/notif_task 生效）+ 服务端 notify_event 实时消费
- BootScreen 分流（首启→引导；常规→自动恢复）
...
### 新增（Features）
- **可选项屏**（`lib/features/options/options_screen.dart`）
  - 84 项分组显示（安全/隐私/功能/界面/语音/AI/数据/连接）
  - **隐私保护模式**一键启用（原「一键最严」）
  - 🔒 安全底线不可改 · ⚠ 危险开关二次确认 · 「需条件」标注
  - 点击切换（走 `options_set`，危险项带 `force`）
- **扩展功能屏**（`lib/features/ext/ext_screens.dart`）
  | 屏 | 对应后端 |
  |---|---|
  | 智能家居 | 场景/区域/围栏/能源/自动化/设备发现 |
  | 监控时间线 | 24h 事件密度柱 + 事件列表 + 存储管理 |
  | 通知中心 | 列表 + 全部已读 + 免打扰切换 |
  | 媒体控制 | 播放器列表 + play/pause/stop/音量 |
  | 知识库 | 上传 + RAG 检索（显示命中与得分） |
- 抽屉菜单新增 6 个入口（智能家居/监控时间线/通知中心/媒体控制/知识库/可选项）

### 实现要点
- 共用 `_ExtMixin`（事件监听 + 命令发送 + 自动清理）
- **无数据一律显示 `--`，不模拟**（与后端约定一致）
- 随系统语言切中英文（`Localizations.localeOf`）

### 验证
- Dart 静态校验 53/53 通过（括号平衡 · 字符串闭合）
- 修正校验器自身缺陷（行注释内括号误算）
...
### 新增（Features）
- **FUI v2 灰白地形语言落地**（先生 09-04 定稿——此前仅升版本号未改 UI，先生指出）：
  - `app_theme.dart` 色板重构：纯黑 → 中深灰（#1E2126/#2B3038）；黑线框 → **白线低透明**（rgba 255 .28/.14）；主强调红 → **功能色绿**（在线/数据）、新增琥珀（未连/警告）——红仅警报
  - `fui_widgets.dart` 新增 **FuiTerrainBackground**（高斯峰海拔场 + Marching Squares 等高线——与 HTML 原型同算法；seed 确定性、白线淡显、仅绘制一次）
- **本地模式启动（先生 09-04 裁决——此前启动仍强制跳连接，先生指出）**：
  - `main.dart` home 改 HomeShell——**开屏不再进 ConnectScreen**，本地直接可用
  - HomeShell：有已存 token **静默自动恢复连接**；无则停留本地；地形背景叠于主内容
  - 抽屉新增「连接主机」入口（绿，注明"对话/同步/远端摄像头需连接"）——需用时手动连（设置→连接与服务器→主机 仍在）
  - ConnectScreen 导航适配双场景：从主界面进入成功 **pop 回**；token 失效独立重登则 pushReplacement
- 版本 0.4.3+17 → **0.4.3+18**

### 说明
- FUI v2 后续深化（全屏地形/开屏 boot 序列/alert_event+weather 消费端/中英双语）下一批
